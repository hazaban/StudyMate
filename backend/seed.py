...
Usage:
  本地运行:  python seed.py  (需要 DATABASE_URL 环境变量指向 Supabase)
  远程API:   POST /api/dev/seed  (部署后通过 Vercel 后端调用)
"""
import sys, os
...
def _408_subjects(today):
    ...
    return [
        ...
        {'name': '政治', 'target_score': '70', 'chapters': [
            ch('马原', 40, 0, 12), ch('毛中特', 45, 13, 28), ch('史纲', 35, 29, 42),
            ch('思修与法基', 30, 43, 55), ch('时政', 25, 56, 70),
        ], 'phases': []},
        {'name': '计算机组成原理', 'target_score': '120', 'chapters': [
            ch('数据表示与运算', 50, 5, 18), ch('指令系统', 55, 19, 34),
            ch('存储系统', 60, 35, 52), ch('CPU与流水线', 50, 53, 68),
        ], 'phases': []},
        {'name': '数学', 'target_score': '130', 'chapters': [
            ch('高等数学基础', 60, 0, 20), ch('线性代数', 55, 21, 40),
            ch('概率论与数理统计', 50, 41, 60), ch('真题专项训练', 60, 61, 80),
        ], 'phases': []},
    ]
...
# ═══════════════════════════════════════════════════════════════════
# 3. Tasks — 考研408 (覆盖过去6天到未来14天，含start_hour和importance)
# ═══════════════════════════════════════════════════════════════════
task408 = []

task_data = [
    # (offset_days, subject, content, type, duration, status, actual, hour, importance)
    # 过去6天（甘特图起始）
    (-6, "数据结构", "链表基础操作复习", "new_study", 45, "completed", 50, 9, "important_urgent"),
    (-6, "英语", "单词Unit1复习", "review", 30, "completed", 35, 14, "urgent_not_important"),
    (-5, "操作系统", "进程概念与状态转换", "new_study", 60, "completed", 55, 8, "important_urgent"),
    (-5, "政治", "绪论部分复习", "review", 30, "completed", 40, 19, "not_important_not_urgent"),
    (-5, "数据结构", "链表错题复习", "mistake", 25, "completed", 30, 20, "important_not_urgent"),
    (-4, "计算机组成原理", "数据表示与运算", "new_study", 60, "completed", 65, 10, "important_not_urgent"),
    (-4, "英语", "单词Unit2复习", "review", 30, "completed", 25, 14, "urgent_not_important"),
    (-3, "计算机网络", "网络体系结构概述", "new_study", 50, "completed", 45, 9, "important_not_urgent"),
    (-3, "操作系统", "进程管理复习", "review", 40, "completed", 50, 15, "important_urgent"),
    (-2, "操作系统", "进程与线程概念", "new_study", 60, "completed", 60, 8, "important_urgent"),
    (-2, "政治", "马原第一章：马克思主义概述", "review", 40, "completed", 45, 19, "not_important_not_urgent"),
    (-2, "数据结构", "栈与队列基础", "new_study", 45, "completed", 40, 14, "important_not_urgent"),
    (-1, "数据结构", "哈希表与冲突解决", "new_study", 60, "completed", 70, 9, "important_urgent"),
    (-1, "英语", "阅读理解专项训练", "review", 45, "completed", 40, 14, "urgent_not_important"),
    (-1, "计算机组成原理", "Cache存储器映射方式", "new_study", 50, "completed", 55, 16, "important_not_urgent"),
    (-1, "计算机网络", "网络协议错题复习", "mistake", 30, "completed", 25, 19, "urgent_not_important"),
    # 今天
    (0, "数据结构", "二叉树遍历算法（前序/中序/后序/层序）", "new_study", 60, "completed", 65, 8, "important_urgent"),
    (0, "数据结构", "两道二叉树遍历真题（前序+中序）", "review", 30, "completed", 25, 10, "important_not_urgent"),
    (0, "英语", "背诵英语单词Unit5（50词）", "new_study", 30, "pending", 0, 14, "urgent_not_important"),
    (0, "政治", "马原第二章：唯物辩证法", "review", 60, "pending", 0, 15, "not_important_not_urgent"),
    (0, "英语", "英语阅读理解错题回顾", "mistake", 45, "pending", 0, 19, "important_urgent"),
    (0, "操作系统", "整理内存管理笔记（分页/分段/段页式）", "new_study", 60, "completed", 55, 20, "important_not_urgent"),
    # 未来1-14天（甘特图结束）
    (1, "数据结构", "图的存储与遍历（DFS/BFS）", "new_study", 90, "pending", 0, 9, "important_urgent"),
    (1, "英语", "英语单词Unit5复习", "review", 30, "pending", 0, 14, "urgent_not_important"),
    (1, "计算机网络", "TCP/IP协议栈详解", "new_study", 60, "pending", 0, 15, "important_not_urgent"),
    (2, "操作系统", "进程调度算法", "new_study", 60, "pending", 0, 10, "important_not_urgent"),
    (2, "数据结构", "二叉树遍历错题复习", "mistake", 30, "pending", 0, 14, "urgent_not_important"),
    (2, "计算机组成原理", "运算器复习", "review", 40, "pending", 0, 19, "important_not_urgent"),
    (3, "数据结构", "图的最短路径算法", "new_study", 80, "pending", 0, 9, "important_urgent"),
    (3, "政治", "马原复习", "review", 50, "pending", 0, 14, "not_important_not_urgent"),
    (4, "操作系统", "内存管理章节复习", "review", 60, "pending", 0, 14, "important_not_urgent"),
    (4, "英语", "长难句解析练习", "new_study", 45, "pending", 0, 19, "urgent_not_important"),
    (5, "政治", "毛中特第一章", "new_study", 60, "pending", 0, 10, "not_important_not_urgent"),
    (5, "计算机网络", "HTTP协议详解", "new_study", 50, "pending", 0, 15, "important_not_urgent"),
    (6, "数据结构", "本周知识点复习", "review", 90, "pending", 0, 9, "important_urgent"),
    (6, "英语", "英语作文模板学习", "new_study", 45, "pending", 0, 15, "urgent_not_important"),
    (7, "数据结构", "查找算法（顺序/折半/B树）", "new_study", 70, "pending", 0, 9, "important_urgent"),
    (7, "计算机组成原理", "存储系统复习", "review", 50, "pending", 0, 14, "important_not_urgent"),
    (8, "操作系统", "文件系统原理", "new_study", 60, "pending", 0, 10, "important_not_urgent"),
    (8, "计算机网络", "网络层错题复习", "mistake", 30, "pending", 0, 15, "urgent_not_important"),
    (9, "数据结构", "排序算法（插入/交换/选择）", "new_study", 80, "pending", 0, 9, "important_urgent"),
    (9, "英语", "单词Unit6复习", "review", 30, "pending", 0, 14, "urgent_not_important"),
    (10, "计算机网络", "应用层协议DNS/HTTP", "new_study", 50, "pending", 0, 14, "important_not_urgent"),
    (10, "政治", "毛中特复习", "review", 40, "pending", 0, 19, "not_important_not_urgent"),
    (11, "计算机组成原理", "指令系统", "new_study", 60, "pending", 0, 10, "important_not_urgent"),
    (11, "操作系统", "文件系统错题", "mistake", 25, "pending", 0, 15, "urgent_not_important"),
    (12, "数据结构", "排序算法复习", "review", 60, "pending", 0, 9, "important_urgent"),
    (13, "操作系统", "设备管理", "new_study", 50, "pending", 0, 14, "important_not_urgent"),
    (13, "英语", "作文练习", "review", 45, "pending", 0, 19, "urgent_not_important"),
    (14, "数据结构", "本周总结复习", "review", 90, "pending", 0, 9, "important_urgent"),
    (14, "计算机网络", "网络安全基础", "new_study", 40, "pending", 0, 14, "important_not_urgent"),
    (14, "政治", "本周错题复习", "mistake", 30, "pending", 0, 19, "not_important_not_urgent"),
]

for offset, subj, content, ttype, dur, status, actual, hour, imp in task_data:
    task408.append(DailyTask(
        id=uuid4(), plan_id=pid408, date=today+timedelta(days=offset),
        type=ttype, subject=subj, content=content, duration=dur, status=status,
...
print(f"""
  Seed complete:
    ...
    Plans:    考研408 + 软考
    Tasks:    {len(task408)} (408) + {len(taskSoft)} (软考) — 覆盖甘特图（过去6天-未来14天）
    ...
    Focus:    {focus_count} records (30 days)
    四象限:   所有importance分类均有数据
    周视图:   start_hour覆盖6:00-23:00""")
