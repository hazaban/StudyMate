...
// Multiple images in one cell flow horizontally (inline-flex wrapping).
...
function imgCell(images) {
  ...
  const items = images.map(img => {
    ...
    if (!src) return '<span style="font-size:10px;color:#999">[图片]</span>'
    return `<a href="${esc(src)}" target="_blank" style="display:inline-block;margin:1px"><img src="${esc(src)}" width="140" height="105" style="border:1px solid #ccc;border-radius:4px" /></a>`
  }).join('')
  return `<div style="display:inline-flex;flex-wrap:wrap;gap:2px;align-items:flex-start">${items}</div>`
}
...
function cardsToExcel(cards, includeAnswer) {
  ...
  for (const c of cards) {
    ...
    if (hasImgQ) html += `<td style="padding:6px;width:290px">${imgCell(c.question_images)}</td>`
    if (includeAnswer) {
      if (hasTextA) html += `<td style="padding:6px;max-width:200px;word-break:break-all">${esc(c.answer)}</td>`
      if (hasImgA) html += `<td style="padding:6px;width:290px">${imgCell(c.answer_images)}</td>`
    }
...
function mistakesToExcel(mistakes, includeAnswer) {
  ...
  for (const m of mistakes) {
    ...
    if (hasImgQ) html += `<td style="padding:6px;width:290px">${imgCell(m.question_images)}</td>`
    if (includeAnswer) {
      if (hasTextA) html += `<td style="padding:6px;max-width:200px;word-break:break-all">${esc(m.answer)}</td>`
      if (hasImgA) html += `<td style="padding:6px;width:290px">${imgCell(m.answer_images)}</td>`
