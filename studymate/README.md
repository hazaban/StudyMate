...
StudyMate 学习星球是一款面向考研/考公/考证备考人群的 **AI 抗遗忘学习工具**。它结合了**艾宾浩斯遗忘曲线**科学的复习调度、**番茄钟专注计时**、**游戏化学习农场**三大核心机制，帮助用户高效记忆、持续专注、快乐学习。
...
### 🏆 推荐方案：Vercel + Render + Supabase + 腾讯云 COS

经过对比，以下是个人使用的最佳方案组合，**全部免费额度即可满足需求**：

| 组件 | 平台 | 费用 | 说明 |
|------|------|------|------|
| 前端 H5 | **Vercel** | 免费 | 全球 CDN，速度快，自动部署 |
| 后端 API | **Render** | 免费 | FastAPI 完整运行，无超时限制 |
| 数据库 | **Supabase** | 免费 | PostgreSQL，稳定可靠，500MB 空间 |
...
| 方案 | 优点 | 缺点 | 适合 |
|------|------|------|------|
| **全 Vercel** | 一个平台管理 | 后端 Serverless 有 10 秒超时，AI 功能可能超时 | 不用 AI 功能 |
| **全 Render** | 一个平台管理 | 前端速度不如 Vercel | 追求简单 |
| **全腾讯云** | 国内速度快 | 要花钱（约50元/月） | 预算充足 |
| **Vercel+Render** ⭐ | 各自最强项组合，全免费 | 两个平台管理 | 个人使用 |

### 详细部署步骤

📖 完整部署文档请查看：[**docs/deploy.md**](docs/deploy.md)

包含从零开始的每一步操作：Supabase 数据库创建、腾讯云 COS 配置、Render 后端部署、Vercel 前端部署、常见问题排查。

---

## 落地方案

### 当前项目可以直接落地使用，以下为三种端侧方案：
...
**优势：**
- 零安装成本，扫码即用
- 一次部署，iOS/Android/PC 通用
- 更新无需审核，即时生效
- 适合 MVP 快速验证

**推荐部署平台：** Vercel（免费）或腾讯云 COS + CDN
...
| 端 | 平台 | 部署方式 | 成本 |
|----|------|----------|------|
| 手机端 | H5 Web | Vercel / 腾讯云 COS | 免费~几十元/月 |
| 手机端 | 微信小程序 | 微信公众平台 | 免费 |
| 电脑端 | H5 Web | 同手机端 | 已包含 |
...
**后端部署：** 推荐使用 [**部署方案**](#部署方案) 中的 Vercel + Render + Supabase 组合。
